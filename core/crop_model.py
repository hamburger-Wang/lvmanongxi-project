# ==================================================
# crop_model.py - 作物分类与长势预测核心模型
# 作用：实现基于卫星影像的作物分类、模型训练和预测功能
# 功能模块：
# 1. 自定义损失函数（IOU、F1、SupCon）
# 2. 多格式数据读取（HDF5、TIFF、IMG）
# 3. 数据生成器（支持不同损失函数模式）
# 4. FCN-3D模型定义（论文实现）
# 5. 模型训练逻辑
# 6. 模型预测逻辑
...
def load_data(file_path):
    ...
    返回:
        tuple - (data, truth)，data为影像数据，truth为标签数据（如果有）
    """
...
def train_model(data_path, out_loss, epochs, model_path, img_path):
    ...
    参数:
        ...
        out_loss: str - 损失函数类型（Cross-entropy/IOU/F1）
        epochs: int - 训练轮数
        model_path: str - 模型保存路径
        img_path: str - 结果图保存路径
    
    返回:
        dict - 训练结果，包含状态、Kappa系数、模型路径等信息
    """
    # 加载数据
    try:
...
def main():
    ...
    parser.add_argument('--model_choice', type=str, default='crop_model',
                        choices=['crop_model', 'crop_model-dry', '第三模型'], help='模型选择')
